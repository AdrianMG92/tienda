...
heredoc;
      if ($this->howMany() > 0){
        ...
        $str .= "</a></td><td>$cantidad</td><td>" .  $producto->getPrecioReal() ." €</td><td>$subtotalTexto €</td><td><a class='btn btn-danger' onclick='return  checkDelete();' href='carro.php?action=delete&id=$key&redirect=".urlencode($_GET['redirect'])."'>Borrar</td></tr>";
        }
      }
      $str .= <<<heredoc
        </tbody>
